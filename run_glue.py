...
def pearson_and_spearman(preds, labels):
    ...
    spearman = spearmanr(preds, labels)
    return {
        "pearson": pearson[0],
        "spearman": spearman[0],
        "spearman_and_pearson": (spearman[0] + pearson[0]) / 2
    }
...
def init_optimizer_and_amp(model, learning_rate, loss_scale, warmup_proportion,
                           num_train_optimization_steps, use_fp16, max_grad_norm, weight_decay):
...
def gen_tensor_dataset(features, is_regression=False):
...
def dump_predictions(path, label_map, preds, examples):
    if label_map is not None:
        ...
        predictions = {
            example.guid: preds[i] for i, example in enumerate(examples)
        }
...
def main(args):
    ...
    if args.do_train:
        ...
        train_data = gen_tensor_dataset(train_features, num_labels == 1)
        if args.local_rank == -1:
            ...
            for step, batch in enumerate(train_iter):
                ...
                if global_step % args.gradient_accumulation_steps == 0:
                    ...
                    if (global_step // args.gradient_accumulation_steps) % args.eval_step == 0 and (args.do_eval or args.do_predict) and is_main_process():
                        ...
                        eval_data = gen_tensor_dataset(eval_features, num_labels == 1)
                        ...
                        if args.do_eval:
                            ...
                            if 'mcc' in results:
                                key = 'mcc'
                            elif 'acc_and_f1' in results:
                                key = 'acc_and_f1'
                            elif 'spearman_and_pearson' in results:
                                key = 'spearman_and_pearson'
