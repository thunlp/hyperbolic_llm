    ...
    def init_data(self, data, tokenizer, max_tokens, gold_level, arg_query_type, argm_query_type):
        for s_id, d in enumerate(tqdm(data, desc='preprocessing')):
            ...
            for i in range(len(predicates)):
                ...
                sentence1i = sentence1[:pre]+[['*']] + sentence1[pre:pre+1]+[['@']]+sentence1[pre+1:]
                args = arguments[i]
                labels = d['plabel'][i]
...
def load_al_data(vocab_file, path, pretrained_model_name_or_path, max_tokens, shuffle, dataset_tag, local_rank=-1, gold_level=0, arg_query_type=0, argm_query_type=0):
    ...
    dataset = AlDataset(path, tokenizer, max_tokens,
                        gold_level, arg_query_type, argm_query_type)
...
def reload_al_data(path, max_tokens, shuffle=False, local_rank=-1):
    dataset = AlDataset()
