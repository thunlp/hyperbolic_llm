...
class RpDataset:
    ...
    def __getitem__(self, i):
        return {'input_ids': self.batch_input_ids[i].long(), 'token_type_ids': torch.zeros_like(self.batch_input_ids[i]).long(), 'attention_mask': self.batch_attention_mask[i].float(), 'target': self.batch_target[i].float()}

    def __len__(self):
        return len(self.batch_input_ids)


def load_rp_data(vocab_file, path, pretrained_model_name_or_path, max_tokens, shuffle, dataset_tag, local_rank=-1):
    ...
    tokenizer = BertTokenizer(
        ...
        do_lower_case=True,
        max_len=512,
        # never_split=("[UNK]", "[SEP]", "[PAD]", "[CLS]", "[MASK]", "<p>", "</p>")
    )
    # tokenizer.add_special_tokens({'additional_special_tokens': ['<p>', '</p>']})
    dataset = RpDataset(path, tokenizer, max_tokens)
...
def reload_rp_data(path, max_tokens, shuffle=False, local_rank=-1):
    dataset = RpDataset()
