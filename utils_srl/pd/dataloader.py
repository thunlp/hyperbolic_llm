...
def load_pd_data(vocab_file, path, pretrained_model_name_or_path, max_tokens, shuffle, local_rank=-1):
    ...
    dataset = PdDataset(path, tokenizer, max_tokens)
...
def reload_pd_data(path, max_tokens, shuffle=False, local_rank=-1):
    dataset = PdDataset()
