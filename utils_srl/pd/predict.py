...
from .dataloader import load_pd_data, reload_pd_data, batch_by_tokens
from modeling import BertForPredicateDisambiguation, BertConfig

nlp = spacy.load('en_core_web_sm')

def dis_lemma(lemma, all_lemmas):
...
def lemmatize(sent, predicates, dis=True, all_lemmas=[]):
    '''
    ...
    if dis:
        plemmas = [dis_lemma(l, all_lemmas) for l in plemmas]
    return plemmas
...
class DisamDataset:
    def __init__(self, lemma_dict, path="", tokenizer=None, max_tokens=1024, all_lemmas=[]):
        if not path:
            return
    ...
    def init_data(self, data, tokenizer, max_tokens):
        for s_id, d in enumerate(tqdm(data, desc='preprocessing')):
            ...
            glemmas = d['lemmas']
            plemmas = lemmatize(sentence, predicates, all_lemmas=self.all_lemmas)
            plemmas1 = lemmatize(sentence, predicates,False, all_lemmas=self.all_lemmas) #don't use edit distance
            ...
            for i in range(len(predicates)):
                ...
                for fid in self.lemma_dict[plemma]:
                    q = self.lemma_dict[plemma][fid]
...
def disam_predict(dataset, model, device, amp, lemma_dict):
    '''
