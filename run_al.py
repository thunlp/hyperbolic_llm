...
from modeling import BertConfig, BertForArgumentLabeling
from utils_srl.al.dataloader import load_al_data, reload_al_data
...
from utils_srl.al.evaluation import evaluation_al
...
def train(args, train_dataloader, dev_dataloader):
    ...
    model = BertForArgumentLabeling(config)
    ...
    for epoch in range(args.max_epochs):
        ...
        if args.eval and args.local_rank < 1:
            score = evaluation_al(model,dev_dataloader,args.amp,device, args.dataset_tag)
...
def test(args, test_dataloader):
    ...
    device = args.local_rank if args.local_rank != -1 else (torch.device('cuda') if torch.cuda.is_available() else torch.device('cpu'))
    model.to(device)
    score = evaluation_al(model, test_dataloader, args.amp, device, args.dataset_tag)
    return score
...
if __name__ == "__main__":
    ...
    if args.train:
        if args.train_path.endswith('.json'):
            train_dataloader = load_al_data(args.vocab_file, args.train_path, args.pretrained_model_name_or_path, args.max_tokens, True, args.dataset_tag, args.local_rank, args.gold_level, args.arg_query_type, args.argm_query_type)
            save_dir = os.path.dirname(args.train_path)+f'/arg_labeling/{args.gold_level}_query_type{args.arg_query_type}.{args.argm_query_type}/train'
            if os.path.exists(save_dir):
                shutil.rmtree(save_dir)
            train_dataloader.dataset.save(save_dir)
            print('training data saved at:', save_dir)
        else:
            train_dataloader = reload_al_data(
                args.train_path, args.max_tokens, True, args.local_rank)
        if not args.eval:
            dev_dataloader = None
        elif args.dev_path.endswith('.json'):
            dev_dataloader = load_al_data(args.vocab_file, args.dev_path, args.pretrained_model_name_or_path, args.max_tokens, False, args.dataset_tag, args.local_rank, args.gold_level, args.arg_query_type, args.argm_query_type)
            save_dir = os.path.dirname(args.train_path)+f'/arg_labeling/{args.gold_level}_query_type{args.arg_query_type}.{args.argm_query_type}/dev/'
            if os.path.exists(save_dir):
                shutil.rmtree(save_dir)
            dev_dataloader.dataset.save(save_dir)
            print('validation data saved at:', save_dir)
        else:
            dev_dataloader = reload_al_data(args.dev_path, args.max_tokens, False, -1)
        checkpoint = None
        if args.local_rank < 1:
            mid = time.strftime("%Y_%m_%d_%H_%M_%S", time.localtime(time.time()))
            if args.tensorboard:
                log_dir = "./logs/{}/argument_labeling/{}".format(args.dataset_tag, mid)
    ...
    if args.test and args.local_rank < 1:
        ...
        for task in task_list:
            if args.dataset_tag == 'conll2005':
                ...
                tag = 'conll12'
            test_path = os.path.join(args.test_path, 'test%s.english.psense.plabel.%s.json' % (task, tag))
            test_dataloader = load_al_data(args.vocab_file, test_path, args.pretrained_model_name_or_path, args.max_tokens, False, args.dataset_tag, args.local_rank, args.gold_level, args.arg_query_type, args.argm_query_type)
            score = test(args, test_dataloader)
            # print("%s: %f" % (task, score))
            result[task] = score
            # writer.add_scalar('%s f1' % (task), score)
        for task, res in result.items():
            print("%s: %s" % (task, str(res)))
