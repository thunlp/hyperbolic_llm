...
from schedulers import PolyWarmUpScheduler, PolyWarmUpSchedulerCorrect
...
class BertPretrainingCriterionNoNSP(torch.nn.Module):
    def __init__(self, vocab_size):
        super(BertPretrainingCriterionNoNSP, self).__init__()
        self.loss_fn = torch.nn.CrossEntropyLoss(ignore_index=-1)
        self.vocab_size = vocab_size
    def forward(self, prediction_scores, masked_lm_labels):
        return self.loss_fn(prediction_scores.view(-1, self.vocab_size), masked_lm_labels.view(-1))
...
def parse_arguments():
    ...
    parser.add_argument("--poly_degree",
                        default=0.5,
                        type=float,
                        help="The degree rate of polynomial learning rate decay.")
    parser.add_argument("--adam_beta1",
                        default=0.9,
                        type=float,
                        help="The beta1 for Adam.")
    parser.add_argument("--adam_beta2",
                        default=0.999,
                        type=float,
                        help="The beta2 for Adam.")
...
def prepare_model_and_optimizer(args, device):
    ...
    no_decay = ['bias', 'gamma', 'beta', 'LayerNorm', 'scale']

    optimizer_grouped_parameters = [
        {'params': [p for n, p in param_optimizer if p.requires_grad and not any(nd in n for nd in no_decay) and not isinstance(p, ManifoldParameter)], 'weight_decay': args.weight_decay},
        {'params': [p for n, p in param_optimizer if p.requires_grad and (any(nd in n for nd in no_decay) or isinstance(p, ManifoldParameter))], 'weight_decay': 0.0}]
    # optimizer = FusedLAMB(optimizer_grouped_parameters,
    #                       lr=args.learning_rate)
    optimizer = RiemannianAdam(optimizer_grouped_parameters, stabilize=10, lr=args.learning_rate, eps=1e-8, max_grad_norm=args.max_grad_norm, betas=(args.adam_beta1, args.adam_beta2))
    lr_scheduler = PolyWarmUpSchedulerCorrect(optimizer,
    ...
    # criterion = BertPretrainingCriterionNoNSP(config.vocab_size)
...
def take_optimizer_step(args, optimizer, model, overflow_buf, global_step):
    ...
    if args.allreduce_post_accumulation:
        ...
        #         if p.requires_grad and (torch.isnan(p.grad.max()) or torch.isinf(p.grad.max())):
        #             abnormal.append((n, p))
        # except:
        #     import pdb; pdb.set_trace()
        # import pdb; pdb.set_trace()
        # if is_main_process():
        #     if _amp_state.loss_scalers[0].loss_scale() < 1:
        #         for n, p in model.named_parameters():
        #             if p.requires_grad and p.grad is not None and (torch.isnan(p.grad.sum()) or torch.isinf(p.grad.sum())):
        #                 print(n)
        #         import pdb; pdb.set_trace()
        optimizer.step()
        #optimizer.zero_grad()
...
def main():
    ...
    if args.do_train:
        ...
        train_count = tqdm(range(int(args.max_steps) // int(args.log_freq)), disable=not is_main_process())
        train_count.update(int(global_step) // int(args.log_freq))
        while True:
            thread = None
            ...
            for f_id in range(f_start_id + 1 , len(files)):
                ...
                train_iter = tqdm(train_dataloader, desc="Iteration", disable=True) if is_main_process() else train_dataloader
                ...
                for step, batch in enumerate(train_iter):
                    ...
                    if global_step >= args.steps_this_run or timeout_sent:
                        ...
                        if is_main_process():
                            ...
                            train_iter.set_description("loss:%f" % final_loss)
