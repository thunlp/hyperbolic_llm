...
def inner(u, v, *, keepdim: bool = False, dim: int = -1):
    r"""
...
def _dist(x, y, k: torch.Tensor, keepdim: bool = False, dim: int = -1):
    ...
    return torch.acosh((d / k).clamp_min(1 + 1e-8))
...
def cdist(x: torch.Tensor, y: torch.Tensor, k: torch.Tensor):
    ...
    return torch.acosh((-(x @ y.transpose(-1, -2))).clamp_min(1 + 1e-6))


@torch.jit.script
def cinner(x: torch.Tensor, y: torch.Tensor):
    x = x.clone()
    x.narrow(-1, 0, 1).mul_(-1)
...
def _norm(u, keepdim: bool = False, dim: int = -1):
    return torch.sqrt((_inner(u, u, keepdim=keepdim)).clamp_min(1e-8))
...
def _logmap0(y, k, dim: int = -1):
    ...
    zero_point[0] = 1
    return torch.acosh(alpha.clamp_min(1 + 1e-8)) / torch.sqrt(alpha * alpha - 1) * (y - alpha * zero_point)
...
def _parallel_transport(x, y, v, k, dim: int = -1):
    ...
    denom = torch.clamp_min(k - _inner(x, y, keepdim=True), 1e-8)
...
def _parallel_transport0(y, v, k, dim: int = -1):
    ...
    # return p
    nom = _inner(y, v, keepdim=True)
    denom = torch.clamp_min(k - _inner0(y, k=k, keepdim=True), 1e-8)
...
def _parallel_transport0back(x, v, k, dim: int = -1):
    ...
    denom = torch.clamp_min(k - _inner0(x, k=k, keepdim=True), 1e-8)
...
def mid_point(x, w):
    ave = w.matmul(x)
    denom = (_inner(ave, ave, keepdim=True))
    # mask = denom < 0
    # denom[mask].clamp_max_(-1e-6)
    # denom[~mask].clamp_min_(1e-6)
