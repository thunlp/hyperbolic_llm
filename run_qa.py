...
from modeling import BertConfig, BertForQuestionAnsweringMRQA
...
def train(args, train_dataset, model, tokenizer):
    ...
    optimizer_grouped_parameters = [
        ...
        {
            'params': [
                p for n, p in model.named_parameters() if any(nd in n for nd in no_decay) or isinstance(p, ManifoldParameter)
            ],
            'weight_decay': 0.0
        },
        ]
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.evaluate_during_training and args.save_steps > 0 and (global_step % args.save_steps == 0):
                    ...
                    if results['f1']>best_f1:
                        ...
                        prev_best = 0
                        result_txt = os.path.join(args.output_dir, 'best_result.txt')
                        if os.path.exists(result_txt):
                            with open(result_txt, 'r') as f:
                                prev_best = float(f.readline().strip())
                        if prev_best < best_f1:
...
def load_and_cache_examples(args, tokenizer, evaluate=False, output_examples=False):
    ...
    cached_features_file = os.path.join(os.path.dirname(input_file), 'cached_{}_{}_{}'.format(
        ...
        list(filter(None, args.init_checkpoint.split('/'))).pop(),
        str(args.max_seq_length)))


    if os.path.exists(cached_features_file) and not args.overwrite_cache and not output_examples:
        if args.local_rank in [0, -1] and not evaluate:
            torch.distributed.barrier()  # Make sure only the first process in distributed training process the dataset, and the others will use the cache
...
def main():
    ...
    model = BertForQuestionAnsweringMRQA(config)
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        model = BertForQuestionAnsweringMRQA(config)
        model.load_state_dict(torch.load(os.path.join(args.output_dir, 'best_model.pt'), 'cpu'))
        model.to(args.device)
        if args.n_gpu > 1:
            model = torch.nn.DataParallel(model)
        # Evaluate
