...
from schedulers import LinearWarmUpScheduler, PolyWarmUpSchedulerCorrect
from apex import amp
from sklearn.metrics import matthews_corrcoef, f1_score
from utils import (is_main_process, mkdir_by_main_process, format_step,
                   get_world_size)
from processors.re import convert_examples_to_features, PROCESSORS
...
def compute_metrics(preds, labels):
    ...
    guess_relation = preds != 0
    gold_relation = labels != 0
    guesses_by_relation = guess_relation.sum()
    gold_by_relation = gold_relation.sum()
    correct_by_relation = (guess_relation & gold_relation & (labels == preds)).sum()
    acc = correct_by_relation / guesses_by_relation
    recall = correct_by_relation / gold_by_relation
    if acc + recall > 0:
...
class MarginLoss(torch.nn.Module):
    ...
    def forward(self, preds, golds):
        correct_preds = preds.gather(-1, golds.unsqueeze(-1))
...
def parse_args(parser=argparse.ArgumentParser()):
    ...
    parser.add_argument("--eval_step",
                        default=50,
                        type=int,
                        help="Do evaluation every n steps.")
    parser.add_argument("--learning_rate",
                        default=5e-5,
                        type=float,
                        help="The initial learning rate for Adam.")
    parser.add_argument("--hyp_learning_rate",
                        default=5e-5,
                        type=float,
                        help="The initial learning rate for Adam.")
...
def init_optimizer_and_amp(model, learning_rate, loss_scale, warmup_proportion,
                           num_train_optimization_steps, use_fp16, max_grad_norm, weight_decay, hyp_learning_rate):
    ...
    optimizer_grouped_parameters = [
        ...
        {
            'params': [
                p for n, p in param_optimizer if any(nd in n for nd in no_decay) and not isinstance(p, ManifoldParameter)
            ],
            'weight_decay': 0.0,
            'lr': learning_rate
        },
        {
            'params': [
                p for n, p in param_optimizer if isinstance(p, ManifoldParameter)
            ],
            'weight_decay': 0.0,
            'lr': hyp_learning_rate
        },
    ]
...
def gen_tensor_dataset(features):
    ...
    all_subj_start_ids = torch.tensor(
        [f.subj_start for f in features],
        dtype=torch.long,
    )
    all_obj_start_ids = torch.tensor(
        [f.obj_start for f in features],
        dtype=torch.long,
    )
...
def main(args):
    ...
    tokenizer = BertTokenizer(
        ...
        max_len=512,
        # never_split=("[UNK]", "[SEP]", "[PAD]", "[CLS]", "[MASK]", "[E1]", "[/E1]", "[E2]", "[/E2]")
    )  # for bert large
    ...
    model = modeling.BertForRelationExtraction(
        config,
    ...
    # loss_fct = MarginLoss(args.margin)
    ...
    if args.do_train:
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            train_iter = tqdm(train_dataloader, desc="Iteration", disable=not is_main_process())
            for step, batch in enumerate(train_iter):
                model.train()
                if args.max_steps > 0 and global_step > args.max_steps:
                    break
                batch = tuple(t.to(device) for t in batch)
                input_ids, input_mask, segment_ids, label_ids, subj_start, obj_start = batch
                logits = model(input_ids, segment_ids, input_mask, subj_start, obj_start)
                loss = loss_fct(
                ...
                global_step += 1
                if global_step % 10 == 0:
                    train_iter.set_description('step:%d, loss:%f' % (global_step + 1, np.mean(losses)))
                    losses = []
                if global_step % args.gradient_accumulation_steps == 0:
                    ...
                    if (global_step // args.gradient_accumulation_steps) % args.eval_step == 0 and (args.do_eval or args.do_predict) and is_main_process():
                        ...
                        for i, (input_ids, input_mask, segment_ids, label_ids, subj_start, obj_start) in tqdm(
                            ...
                            subj_start = subj_start.to(device)
                        ...
                        if args.do_eval:
                            ...
                            eval_result = compute_metrics(preds, out_label_ids)
                            results.update(eval_result)
                            key = 'f1'
                            if 'best_result' not in results or results[key] > results['best_result']:
                                results['best_result'] = results[key]
    ...
    dllogger.flush()
    if is_main_process():
        test_result = test(args, model, processor, tokenizer, num_labels, loss_fct, results, device)
...
def test(args, model, processor, tokenizer, num_labels, loss_fct, results, device):
    ...
    model_to_load.load_state_dict(torch.load(os.path.join(args.output_dir, modeling.WEIGHTS_NAME), 'cpu')['model'])
    model.eval()
    preds = None
    out_label_ids = None
    eval_loss = 0
