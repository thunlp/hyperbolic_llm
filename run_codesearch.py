...
from codesearch_utils import (compute_metrics, convert_examples_to_features,
...
def train(args, train_dataset, model, tokenizer, optimizer, num_labels):
    """ Train the model """
    ...
    for idx, _ in enumerate(train_iterator):
        tr_loss = 0.0
        for step, batch in tqdm(enumerate(train_dataloader), total=len(train_dataloader), disable=args.local_rank not in [-1, 0]):
        ...
        if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
            results = evaluate(args, model, tokenizer, num_labels, checkpoint=str(args.start_epoch + idx))
            ...
            torch.save(model.state_dict(), os.path.join(last_output_dir, "model.pt"))
...
def evaluate(args, model, tokenizer, num_labels, checkpoint=None, prefix="", mode='dev'):
...
def main():
    ...
    if args.do_train:
        ...
        global_step, tr_loss = train(args, train_dataset, model, tokenizer, optimizer, num_labels)
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        result = evaluate(args, model, tokenizer, num_labels, checkpoint="best_model.pt", prefix=global_step)
    ...
    if args.do_predict:
        ...
        model_dict = torch.load(os.path.join(args.pred_model_dir, "best_model.pt"), 'cpu')
        model_to_load = model.module if hasattr(model, 'module') else model
        model_to_load.load_state_dict(model_dict)
        model.to(args.device)
        evaluate(args, model, tokenizer, num_labels, checkpoint=None, prefix='', mode='test')
    return results
